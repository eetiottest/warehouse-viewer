...
if event.selection.get("rows"):
    ...
    # Transpose to get keys as rows, reset index to avoid numbers, and use write() to avoid the "Value" header
    clean_details = details.to_frame().transpose().melt().set_index('variable')
    
    with st.expander(f"Details for: {selected_row.get('Location')}", expanded=True):
        st.write(clean_details.rename(columns={'value': ''}))
